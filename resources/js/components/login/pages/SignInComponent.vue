    ...
    <div class="wrapper">
        ...
        <div class="container">
            <form @submit.prevent="onSubmit">
                ...
                <div v-if="errors.nameOrpassword" class="error-text">{{ errors.nameOrpassword[0] }}</div>
                <button type="submit">SIGN IN</button>
            </form>
            <div class="divider">__________</div>
            <router-link to="/" class="login-link">Create an account</router-link>
        </div>
    </div>
</template>
    ...
    methods: {
        onSubmit() {
            ...
            this.$store.dispatch('signin', this.formData)
                .then(() => {
                    const isAdmin = this.$store.state.auth.me.is_admin
